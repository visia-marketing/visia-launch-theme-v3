...
// The body wraps the heading and the hover panel, so the link's accessible name was the
// heading, the description and the pseudo-button concatenated. Naming it after the card
// keeps the announcement short and unique. WCAG 2.4.4.
...
<div class="uk-card card-background--image cards-style--overlay <?php echo esc_attr( $extra_class ); ?>"<?php echo $aos_attrs; ?>>
    <div class="uk-height-1-1 uk-flex uk-flex-column uk-position-relative uk-card--inner">
        ...
        <?php // uk-flex-right bottom-aligns the body's content over the image.
              // hoverCardsInit() in main.js keeps aria-expanded on this element in sync and
              // opens the panel on focus as well as hover. ?>
        <?php if ( $is_link ) : ?>
        <a href="<?php echo esc_url( $card_url ); ?>" class="card-body uk-card-body uk-flex uk-flex-column uk-flex-right uk-flex-top uk-height-1-1" aria-label="<?php echo esc_attr( $link_name ); ?>"<?php echo visia_new_window_attrs( $new_window ); ?>>
        <?php else : ?>
        <div class="card-body uk-card-body uk-flex uk-flex-column uk-flex-right uk-flex-top uk-height-1-1">
        <?php endif; ?>
